...
```{r}
dat_sample <- data.frame(dat[sample(1:nrow(dat),1000,replace=FALSE),])
```
...
```{r}
summary <- expand.grid(c("High", "Medium", "Low"), c(1, 0, -1))
colnames(summary) <- c("Income", "Correlation")

tmp <- dat_bathroom %>% group_by(Income, math_corr) %>% summarise(Correlation = math_corr, bathroom_math_count = n())
summary <- merge(summary, as.data.frame(tmp)[-2], by=c("Income", "Correlation"), all.x = TRUE)
tmp <- dat_bathroom %>% group_by(Income, read_corr) %>% summarise(Correlation = read_corr, bathroom_read_count = n())
summary <- merge(summary, as.data.frame(tmp)[-2], by=c("Income", "Correlation"), all.x = TRUE)
tmp <- dat_bathroom %>% group_by(Income, sci_corr) %>% summarise(Correlation = sci_corr, bathroom_sci_count = n())
summary <- merge(summary, as.data.frame(tmp)[-2], by=c("Income", "Correlation"), all.x = TRUE)

tmp <- dat_car %>% group_by(Income, math_corr) %>% summarise(Correlation = math_corr, car_math_count = n())
summary <- merge(summary, as.data.frame(tmp)[-2], by=c("Income", "Correlation"), all.x = TRUE)
tmp <- dat_car %>% group_by(Income, read_corr) %>% summarise(Correlation = read_corr, car_read_count = n())
summary <- merge(summary, as.data.frame(tmp)[-2], by=c("Income", "Correlation"), all.x = TRUE)
tmp <- dat_car %>% group_by(Income, sci_corr) %>% summarise(Correlation = sci_corr, car_sci_count = n())
summary <- merge(summary, as.data.frame(tmp)[-2], by=c("Income", "Correlation"), all.x = TRUE)

tmp <- dat_cell %>% group_by(Income, math_corr) %>% summarise(Correlation = math_corr, cell_math_count = n())
summary <- merge(summary, as.data.frame(tmp)[-2], by=c("Income", "Correlation"), all.x = TRUE)
tmp <- dat_cell %>% group_by(Income, read_corr) %>% summarise(Correlation = read_corr, cell_read_count = n())
summary <- merge(summary, as.data.frame(tmp)[-2], by=c("Income", "Correlation"), all.x = TRUE)
tmp <- dat_cell %>% group_by(Income, sci_corr) %>% summarise(Correlation = sci_corr, cell_sci_count = n())
summary <- merge(summary, as.data.frame(tmp)[-2], by=c("Income", "Correlation"), all.x = TRUE)

tmp <- dat_pc %>% group_by(Income, math_corr) %>% summarise(Correlation = math_corr, pc_math_count = n())
summary <- merge(summary, as.data.frame(tmp)[-2], by=c("Income", "Correlation"), all.x = TRUE)
tmp <- dat_pc %>% group_by(Income, read_corr) %>% summarise(Correlation = read_corr, pc_read_count = n())
summary <- merge(summary, as.data.frame(tmp)[-2], by=c("Income", "Correlation"), all.x = TRUE)
tmp <- dat_pc %>% group_by(Income, sci_corr) %>% summarise(Correlation = sci_corr, pc_sci_count = n())
summary <- merge(summary, as.data.frame(tmp)[-2], by=c("Income", "Correlation"), all.x = TRUE)

tmp <- dat_tv %>% group_by(Income, math_corr) %>% summarise(Correlation = math_corr, tv_math_count = n())
summary <- merge(summary, as.data.frame(tmp)[-2], by=c("Income", "Correlation"), all.x = TRUE)
tmp <- dat_tv %>% group_by(Income, read_corr) %>% summarise(Correlation = read_corr, tv_read_count = n())
summary <- merge(summary, as.data.frame(tmp)[-2], by=c("Income", "Correlation"), all.x = TRUE)
tmp <- dat_tv %>% group_by(Income, sci_corr) %>% summarise(Correlation = sci_corr, tv_sci_count = n())
summary <- merge(summary, as.data.frame(tmp)[-2], by=c("Income", "Correlation"), all.x = TRUE)
...
```{r}
write.table(dat, file='generated_data/dat_simplified.tsv', quote = FALSE, sep='\t', row.names = FALSE)
...
write.table(dat_car, file='generated_data/dat_car.tsv', quote = FALSE, sep='\t', row.names = FALSE)
write.table(dat_cell, file='generated_data/dat_cell.tsv', quote = FALSE, sep='\t', row.names = FALSE)
write.table(dat_pc, file='generated_data/dat_pc.tsv', quote = FALSE, sep='\t', row.names = FALSE)
write.table(dat_tv, file='generated_data/dat_tv.tsv', quote = FALSE, sep='\t', row.names = FALSE)
```

```{r}
write.table(dt_bathroom, file='generated_data/dat_bathroom.tsv', quote = FALSE, sep='\t', row.names = FALSE)
write.table(dt_car, file='generated_data/dat_car.tsv', quote = FALSE, sep='\t', row.names = FALSE)
write.table(dt_cell, file='generated_data/dat_cell.tsv', quote = FALSE, sep='\t', row.names = FALSE)
write.table(dt_pc, file='generated_data/dat_pc.tsv', quote = FALSE, sep='\t', row.names = FALSE)
write.table(dt_tv, file='generated_data/dat_tv.tsv', quote = FALSE, sep='\t', row.names = FALSE)
```
